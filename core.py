...
class Core(QObject):
    ...
    def when_time_to_backup(self):
        ...
        if self.server.is_running and self.auto_backup:
            ...
            self.backup_manager.run_task(self.backup_manager.auto_task)
    ...
    def when_detected_player_cmd(self,line):
        ...
        try:
            ...
            self.out(ROLE,"WARN",f"\"{line}\" has option which is not in valid JSON format")
            return
    ...
    def core_exec(self,command,option):
        if command=="backup":
            ...
            if action==None:
                self.out(ROLE,"WARN",f"\"action\" option for \"backup\" command is missing")
                return
            elif action=="new":
                tag=option.get("tag")
                self.is_backing_up=True
                if tag==None:
                    ...
                    self.backup_manager.run_task(self.backup_manager.new_tagged_backup,(tag,))
            # elif action=="rm":
            #     name=option.get("name")
            #     if name==None:
            #         self.out(ROLE,"WARN",f"\"name\" option for \"backup rm\" command is missing")
            #         return
            #     else:
            #         self.is_backing_up=True
            #         self.backup_manager.run_task(self.backup_manager.rm_backup,(name,))
            elif action=="cl":
                self.is_backing_up=True
                self.backup_manager.run_task(self.backup_manager.clean)
            elif action == "ls":
                auto_commits = (
                    self.backup_manager.get_commit_msg_by_msg_prefix(
                ...
                name=option.get("name")
                if name==None:
                    self.out(ROLE,"WARN",f"\"name\" option for \"backup restore\" command is missing")
                    return
                else:
                    self.is_backing_up=True
                    ...
                    self.backup_manager.run_task(self.backup_manager.new_branch,(name,))
                    if restart_later:
                        self.start_server()
    ...
    def exec(self,command):
        if command.startswith("$"):
            ...
            try:
                ...
                self.out(ROLE,"WARN",f"\"{command}\" has option which is not in valid JSON format")
                return
            self.core_exec(command,option)
        else:
